...
def load_rn_shp(path, is_directed=True):
    ...
    for u, v, data in g.edges(data=True):
        ...
        eid = data['eid']
        res_dict[eid] = {}
        res_dict[eid]['coords'] = res_coords
        res_dict[eid]['length'] = sum([distance(coords[i], coords[i+1]) for i in range(len(coords) - 1)])
        res_dict[eid]['level'] = data['highway']
